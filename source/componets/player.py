...
class Player(pygame.sprite.Sprite):
    ...
    def setup_velocities(self):
        speed = self.player_data.get('speed')
        self.x_vel = 0
        self.y_vel = 0

        self.max_walk_vel = speed.get('max_walk_speed')
        self.max_run_vel = speed.get('max_run_speed')
        self.max_y_vel = speed.get('max_y_velocity')
        self.jump_vel = speed.get('jump_velocity')
        self.walk_accel = speed.get('walk_accel')
        self.run_accel = speed.get('run_accel')
        self.turn_accel = speed.get('turn_accel')
    ...
    def load_images(self):
        sheet = setup.GRAPHICS.get('mario_bros')
        frame_rects = self.player_data.get('image_frames')
        ...
        for gruop, group_frame_rects in frame_rects.items():
            for frame_rect in group_frame_rects:
                right_image = tools.get_image(sheet, frame_rect.get('x'), frame_rect.get('y'), frame_rect.get('width'), frame_rect.get('height'), (0, 0, 0), C.PLAYER_MULTI)
                left_image = pygame.transform.flip(right_image, True, False)
                if gruop == 'right_small_normal':
                    self.right_small_normal_frames.append(right_image)
                    self.left_small_normal_frames.append(left_image)
                if gruop == 'right_big_normal':
                    self.right_big_normal_frames.append(right_image)
                    self.left_big_normal_frames.append(left_image)
                if gruop == 'right_big_fire':
